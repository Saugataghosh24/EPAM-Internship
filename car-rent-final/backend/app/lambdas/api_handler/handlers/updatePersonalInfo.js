  ...
  try {
    ...
    if (hasFile && fileBuffer) {
      try {
        ...
        const fileSize = fileBuffer.length / 1024; // Size in KB
        
        // Check file size - MongoDB has a 16MB document limit
        if (fileSize > 5000) { // 5MB warning threshold
          console.warn(`Large image detected (${Math.round(fileSize)}KB). Consider resizing for better performance.`);
        }
        
        // Convert to base64 and store directly in MongoDB
        const base64Image = convertToBase64(fileBuffer, fileMimeType);
        updateData.imageUrl = base64Image;
        console.log(`Image converted to base64 (${Math.round(fileSize)}KB)`);
      } catch (conversionError) {
        console.error("Failed to convert image to base64:", conversionError);
        // Continue with the update even if image conversion fails
      }
    }
    ...
    console.log("Updating user with data:", JSON.stringify({
      ...updateData,
      imageUrl: updateData.imageUrl ? '[BASE64_IMAGE_STRING]' : null // Don't log the full base64 string
    }));
